...
# -*- coding: utf-8 -*-
"""
PNG 맵 좌표·방향 등록기 (Tkinter + ROS YAML 지원) — v1.1
- 파일 선택 대화상자 필터 수정: "*.yaml;*.yml" → "*.yaml *.yml *.YAML *.YML"
- 이미지 필터도 대문자 확장자 포함 및 공백 구분으로 수정
"""
import os
import math
import time
...
class MapMarkerApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("PNG 맵 좌표·방향 등록기 (YAML 지원) v1.1")
    ...
    def on_save(self):
        ...
        try:
            with open(path, "w", encoding="utf-8") as f:
                f.write("# Saved by map_marker_app_yaml.py v1.1\n")
                if self.yaml_path:

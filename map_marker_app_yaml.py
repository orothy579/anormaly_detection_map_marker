...
# -*- coding: utf-8 -*-
"""
PNG 맵 좌표·방향 등록기 (Tkinter + ROS YAML 지원)
- ROS map_server YAML을 불러와서 resolution, origin, image를 읽고 자동 적용
- PNG/JPG를 수동으로 불러올 수도 있음
- 캔버스에서 "좌클릭 후 드래그"로 화살표(좌표+방향) 마커를 무제한 등록 (중첩 제한 없음)
- Save 시 픽셀 좌표 + 월드(map) 좌표를 모두 텍스트로 저장
좌표계/각도 정의:
- 이미지 픽셀: 오른쪽 +X, 아래 +Y
- map 좌표: 오른쪽 +X, 위 +Y
- 각도: 이미지 0°=오른쪽(+X), 90°=아래(+Y)
        map 0°=오른쪽(+X), 90°=위(+Y) → 변환 시 θ_map = wrap(-θ_img + origin_yaw)
필요 패키지:
  pip install pillow pyyaml
실행:
  python3 map_marker_app_yaml.py
"""
import os
import math
import time
...
class MapMarkerApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("PNG 맵 좌표·방향 등록기 (YAML 지원)")
    ...
    def _build_ui(self):
        ...
        ttk.Checkbutton(left, text="셀 중심 좌표로 저장(+0.5 오프셋)", variable=self.use_center).grid(row=9, column=0, sticky="w", pady=(6, 12))
        ttk.Label(left, text="캔버스: 좌클릭 후 드래그 → 방향 지정", foreground="#666").grid(row=10, column=0, sticky="w", pady=(0, 12))

        # --- Save/Clear ---
        ttk.Label(left, text="4) 저장 / 관리", font=("", 11, "bold")).grid(row=11, column=0, sticky="w", pady=(0, 6))
        btns = ttk.Frame(left)
        btns.grid(row=12, column=0, sticky="ew")
        ...
        ttk.Label(left, text="각도: 0°→오른쪽(+X), 90°→아래(+Y) / map은 위(+Y)", foreground="#666").grid(row=13, column=0, sticky="w", pady=(10, 0))

        # --- Marker list ---
        ttk.Label(left, text="마커 목록", font=("", 11, "bold")).grid(row=14, column=0, sticky="w", pady=(12, 6))
        self.marker_list = tk.Listbox(left, height=18)
        self.marker_list.grid(row=15, column=0, sticky="nsew")
        ttk.Button(left, text="선택 삭제", command=self.on_delete_selected).grid(row=16, column=0, sticky="ew", pady=(6, 0))
    ...
    def on_open_yaml(self):
        ...
        meta = MapMeta()
        try:
            meta.resolution = float(data.get("resolution", meta.resolution))
            origin = data.get("origin", [0.0, 0.0, 0.0])
            if not isinstance(origin, (list, tuple)) or len(origin) < 3:
                raise ValueError("origin 필드는 [x, y, yaw] 리스트여야 합니다.")
            meta.origin_x = float(origin[0])
            meta.origin_y = float(origin[1])
            meta.origin_yaw = float(origin[2])  # 이미 라디안 기준이라고 가정
            meta.image_path = data.get("image", None)
            meta.negate = data.get("negate", None)
            meta.occupied_thresh = data.get("occupied_thresh", None)
            meta.free_thresh = data.get("free_thresh", None)
        except Exception as e:
        ...
        # If image path present: resolve relative to yaml
    ...
    def on_open_image_manual(self):
        path = filedialog.askopenfilename(title="이미지 열기", filetypes=[("Image", "*.png;*.jpg;*.jpeg;*.bmp")])
        if not path:
            return
    ...
    def _load_image(self, path: str):
        ...
        self.canvas.config(width=self.img_size[0], height=self.img_size[1], scrollregion=(0, 0, self.img_size[0], self.img_size[1]))
    ...
    def on_save(self):
        ...
        W, H = self.img_size
        meta = self.meta  # may be defaults if YAML not loaded
        ...
        try:
            with open(path, "w", encoding="utf-8") as f:
                f.write("# Saved by map_marker_app_yaml.py\n")
                if self.yaml_path:
